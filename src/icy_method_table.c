// This file is auto generated by table_compiler
#ifndef TABLE_COMPILER_INDEX
#define TABLE_COMPILER_INDEX
#define array_element_size(array) sizeof(array[0])
#define array_count(array) (sizeof(array)/array_element_size(array))
#include "icydb.h"
#include <stdlib.h>
#endif


icy_method_table * icy_method_table_create(const char * optional_name){
  static const char * const column_names[] = {(char *)"id", (char *)"procedure"};
  static const char * const column_types[] = {"method_id", "method"};
  icy_method_table * instance = calloc(sizeof(icy_method_table), 1);
  icy_table_init((icy_table * )instance, optional_name, 2, (unsigned int[]){sizeof(method_id), sizeof(method)}, (char *[]){(char *)"id", (char *)"procedure"});
  instance->column_names = (char **)column_names;
  instance->column_types = (char **)column_types;
  return instance;
}

void icy_method_table_insert(icy_method_table * table, method_id * id, method * procedure, size_t count){
  void * array[] = {(void* )id, (void* )procedure};
  icy_table_inserts((icy_table *) table, array, count);
}

void icy_method_table_set(icy_method_table * table, method_id id, method procedure){
  void * array[] = {(void* )&id, (void* )&procedure};
  icy_table_inserts((icy_table *) table, array, 1);
}

void icy_method_table_lookup(icy_method_table * table, method_id * keys, size_t * out_indexes, size_t count){
  icy_table_finds((icy_table *) table, keys, out_indexes, count);
}

void icy_method_table_remove(icy_method_table * table, method_id * keys, size_t key_count){
  size_t indexes[key_count];
  size_t index = 0;
  size_t cnt = 0;
  while(0 < (cnt = icy_table_iter((icy_table *) table, keys, key_count, NULL, indexes, array_count(indexes), &index))){
    icy_table_remove_indexes((icy_table *) table, indexes, cnt);
    index = 0;
  }
}

void icy_method_table_clear(icy_method_table * table){
  icy_table_clear((icy_table *) table);
}

void icy_method_table_unset(icy_method_table * table, method_id key){
  icy_method_table_remove(table, &key, 1);
}

bool icy_method_table_try_get(icy_method_table * table, method_id * id, method * procedure){
  void * array[] = {(void* )id, (void* )procedure};
  void * column_pointers[] = {(void *)table->id, (void *)table->procedure};
  size_t __index = 0;
  icy_table_finds((icy_table *) table, array[0], &__index, 1);
  if(__index == 0) return false;
  unsigned int sizes[] = {sizeof(method_id), sizeof(method)};
  for(int i = 1; i < 2; i++){
    if(array[i] != NULL)
      memcpy(array[i], column_pointers[i] + __index * sizes[i], sizes[i]); 
  }
  return true;
}

void icy_method_table_print(icy_method_table * table){
  icy_table_print((icy_table *) table);
}

size_t icy_method_table_iter(icy_method_table * table, method_id * keys, size_t keycnt, method_id * optional_keys_out, size_t * indexes, size_t cnt, size_t * iterator){
  return icy_table_iter((icy_table *) table, keys, keycnt, optional_keys_out, indexes, cnt, iterator);

}
