...
control_to_indexes * control_to_indexes_create(const char * optional_name){
  ...
  static const char * const column_types[] = {"icy_control", "icy_indexes"};
  control_to_indexes * instance = calloc(sizeof(control_to_indexes), 1);
  icy_table_init((icy_table * )instance, optional_name, 2, (unsigned int[]){sizeof(icy_control), sizeof(icy_indexes)}, (char *[]){(char *)"key", (char *)"value"});
...
void control_to_indexes_unset(control_to_indexes * table, icy_control key){
  control_to_indexes_remove(table, &key, 1);
}
...
bool control_to_indexes_try_get(control_to_indexes * table, icy_control * key, icy_indexes * value){
  ...
  unsigned int sizes[] = {sizeof(icy_control), sizeof(icy_indexes)};
  for(int i = 1; i < 2; i++){
    if(array[i] != NULL)
