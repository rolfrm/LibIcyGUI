...
control_to_control * control_to_control_create(const char * optional_name){
  ...
  control_to_control * instance = calloc(sizeof(control_to_control), 1);
  icy_table_init((icy_table * )instance, optional_name, 2, (unsigned int[]){sizeof(icy_control), sizeof(icy_control)}, (char *[]){(char *)"key", (char *)"value"});
...
void control_to_control_unset(control_to_control * table, icy_control key){
  control_to_control_remove(table, &key, 1);
}
