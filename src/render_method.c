// This file is auto generated by table_compiler
#ifndef TABLE_COMPILER_INDEX
#define TABLE_COMPILER_INDEX
#define array_element_size(array) sizeof(array[0])
#define array_count(array) (sizeof(array)/array_element_size(array))
#include "icydb.h"
#include <stdlib.h>
#endif


render_method * render_method_create(const char * optional_name){
  static const char * const column_names[] = {(char *)"control", (char *)"render"};
  static const char * const column_types[] = {"icy_control", "render_control"};
  render_method * instance = calloc(sizeof(render_method), 1);
  icy_table_init((icy_table * )instance, optional_name, 2, (unsigned int[]){sizeof(icy_control), sizeof(render_control)}, (char *[]){(char *)"control", (char *)"render"});
  instance->column_names = (char **)column_names;
  instance->column_types = (char **)column_types;
  return instance;
}

void render_method_insert(render_method * table, icy_control * control, render_control * render, size_t count){
  void * array[] = {(void* )control, (void* )render};
  icy_table_inserts((icy_table *) table, array, count);
}

void render_method_set(render_method * table, icy_control control, render_control render){
  void * array[] = {(void* )&control, (void* )&render};
  icy_table_inserts((icy_table *) table, array, 1);
}

void render_method_lookup(render_method * table, icy_control * keys, size_t * out_indexes, size_t count){
  icy_table_finds((icy_table *) table, keys, out_indexes, count);
}

void render_method_remove(render_method * table, icy_control * keys, size_t key_count){
  size_t indexes[key_count];
  size_t index = 0;
  size_t cnt = 0;
  while(0 < (cnt = icy_table_iter((icy_table *) table, keys, key_count, NULL, indexes, array_count(indexes), &index))){
    icy_table_remove_indexes((icy_table *) table, indexes, cnt);
    index = 0;
  }
}

void render_method_clear(render_method * table){
  icy_table_clear((icy_table *) table);
}

void render_method_unset(render_method * table, icy_control key){
  render_method_remove(table, &key, 1);
}

bool render_method_try_get(render_method * table, icy_control * control, render_control * render){
  void * array[] = {(void* )control, (void* )render};
  void * column_pointers[] = {(void *)table->control, (void *)table->render};
  size_t __index = 0;
  icy_table_finds((icy_table *) table, array[0], &__index, 1);
  if(__index == 0) return false;
  unsigned int sizes[] = {sizeof(icy_control), sizeof(render_control)};
  for(int i = 1; i < 2; i++){
    if(array[i] != NULL)
      memcpy(array[i], column_pointers[i] + __index * sizes[i], sizes[i]); 
  }
  return true;
}

void render_method_print(render_method * table){
  icy_table_print((icy_table *) table);
}

size_t render_method_iter(render_method * table, icy_control * keys, size_t keycnt, icy_control * optional_keys_out, size_t * indexes, size_t cnt, size_t * iterator){
  return icy_table_iter((icy_table *) table, keys, keycnt, optional_keys_out, indexes, cnt, iterator);

}
