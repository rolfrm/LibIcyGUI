...
function_type_inv_table * function_type_inv_table_create(const char * optional_name){
  ...
  static const char * const column_types[] = {"icy_control", "function_type"};
  function_type_inv_table * instance = calloc(sizeof(function_type_inv_table), 1);
  icy_table_init((icy_table * )instance, optional_name, 2, (unsigned int[]){sizeof(icy_control), sizeof(function_type)}, (char *[]){(char *)"key", (char *)"value"});
...
void function_type_inv_table_unset(function_type_inv_table * table, icy_control key){
  function_type_inv_table_remove(table, &key, 1);
}
...
bool function_type_inv_table_try_get(function_type_inv_table * table, icy_control * key, function_type * value){
  ...
  unsigned int sizes[] = {sizeof(icy_control), sizeof(function_type)};
  for(int i = 1; i < 2; i++){
    if(array[i] != NULL)
