...
control_to_int * control_to_int_create(const char * optional_name){
  ...
  static const char * const column_types[] = {"icy_control", "int"};
  control_to_int * instance = calloc(sizeof(control_to_int), 1);
  icy_table_init((icy_table * )instance, optional_name, 2, (unsigned int[]){sizeof(icy_control), sizeof(int)}, (char *[]){(char *)"key", (char *)"value"});
...
void control_to_int_unset(control_to_int * table, icy_control key){
  control_to_int_remove(table, &key, 1);
}
...
bool control_to_int_try_get(control_to_int * table, icy_control * key, int * value){
  ...
  unsigned int sizes[] = {sizeof(icy_control), sizeof(int)};
  for(int i = 1; i < 2; i++){
    if(array[i] != NULL)
