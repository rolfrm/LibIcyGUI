...
control_to_vec4 * control_to_vec4_create(const char * optional_name){
  ...
  static const char * const column_types[] = {"icy_control", "vec4"};
  control_to_vec4 * instance = calloc(sizeof(control_to_vec4), 1);
  icy_table_init((icy_table * )instance, optional_name, 2, (unsigned int[]){sizeof(icy_control), sizeof(vec4)}, (char *[]){(char *)"key", (char *)"value"});
...
void control_to_vec4_unset(control_to_vec4 * table, icy_control key){
  control_to_vec4_remove(table, &key, 1);
}
...
bool control_to_vec4_try_get(control_to_vec4 * table, icy_control * key, vec4 * value){
  ...
  unsigned int sizes[] = {sizeof(icy_control), sizeof(vec4)};
  for(int i = 1; i < 2; i++){
    if(array[i] != NULL)
